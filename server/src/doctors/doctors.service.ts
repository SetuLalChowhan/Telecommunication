...
export class DoctorService {
  ...
  async updateMyProfile(
    ...
    if (dto.slug) {
      sanitizedSlug = sanitizeDoctorSlug(dto.slug);
      ...
      const generatedSlug = generateDoctorSlug(dto.name);

      // The URL follows the display name only while it is still the
      // auto-generated one. Once a doctor deliberately picks a slug, a rename
      // must not silently overwrite it.
      const autoBase = generateDoctorSlug(profile.user?.name);
      const stillAutoGenerated =
        !profile.slug ||
        profile.slug === autoBase ||
        isNumberedSlugFor(profile.slug, autoBase);

      if (stillAutoGenerated && generatedSlug !== profile.slug) {
        // Deterministic de-duplication (`dr-ithika-2`) instead of a random or
        // timestamp suffix, so a rename still produces a clean, guessable URL.
        const taken = await this.repo.findSlugsStartingWith(
          generatedSlug,
          profile.id,
        );
        sanitizedSlug = firstFreeSlug(generatedSlug, taken);
      }
    }
  ...
  /**
   * Real-time availability check for the profile form's public-URL field.
   *
   * Scoped to the signed-in doctor, which both keeps the check rate-limited to
   * real users and lets a doctor's *own* current slug read as available.
   */
  async checkSlugAvailability(userId: string, rawSlug: string) {
    const profile = await this.getOwnProfileOrThrow(userId);
    const slug = sanitizeDoctorSlug(rawSlug || '');

    if (!slug) {
      return { slug: '', available: false, reason: 'INVALID', suggestion: '' };
    }

    const taken = await this.repo.findSlugsStartingWith(slug, profile.id);

    if (!taken.includes(slug)) {
      return { slug, available: true, reason: null, suggestion: slug };
    }
    ...
    return {
      ...
      available: false,
      reason: 'TAKEN',
      suggestion: firstFreeSlug(slug, taken),
    };
  }
